...
const Bytenode = {
  ...
  compileFiles ({
    ...
    assert(!createLoader || loaderPattern.includes('%'),
      new Error('loaderPattern must contain "%" character.'));
  ...
  run ({ bytecode, filename }) {
    ...
    // Create a context that includes `require`
    const context = vm.createContext({
      require,
      console, // Allow console if needed
      module, // Optionally provide `module`
      __dirname, // Optionally provide directory-related globals
      __filename // Optionally provide filename
    });
  ...
  registerExtension (ext) {
    ...
    Module._extensions[ext] = function BytenodeRequire (mod, filename) {
      const result = Bytenode.run({ filename });
      if (typeof result === 'function') {
        return result.apply(mod.exports, [
          mod.exports,
        ...
        // Handle the case where result is not a function
        mod.exports = result; // If it's just a value, assign it directly
      }
